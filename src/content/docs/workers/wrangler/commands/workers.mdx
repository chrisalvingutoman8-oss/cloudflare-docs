...
Create or update a Preview for branch and pull request work. Use [`wrangler deploy`](#deploy) for production.

```txt
wrangler preview [OPTIONS]
```

- `--name` <Type text="string" /> <MetaInfo text="optional" />
  - The name of the Preview to create or update.
...
Delete one Preview and its deployments.

```txt
wrangler preview delete --name <preview-name> [OPTIONS]
```

- `--name` <Type text="string" /> <MetaInfo text="required" />
  - The name of the Preview to delete.

---

### `preview secret put`

Create or update a secret on a Preview. This patches the named Preview's latest deployment and creates a new deployment that goes live immediately.

```txt
wrangler preview secret put SECRET_NAME [OPTIONS]
```

- `SECRET_NAME` <Type text="string" /> <MetaInfo text="required" />
  - The name of the secret to add or update.
- `--name` <Type text="string" /> <MetaInfo text="optional" />
  - The name of the Preview to update. Defaults to the current git branch.
- `--message` <Type text="string" /> <MetaInfo text="optional" />
  - A descriptive message for this deployment.
- `--tag` <Type text="string" /> <MetaInfo text="optional" />
  - A tag for this deployment.

---

### `preview secret delete`

Delete a secret from a Preview. This patches the named Preview's latest deployment and creates a new deployment that goes live immediately.

```txt
wrangler preview secret delete SECRET_NAME [OPTIONS]
```
...
List all secrets on a Preview. Reads from the named Preview's latest deployment. Secret values are masked in the output.

```txt
wrangler preview secret list [OPTIONS]
```

- `--name` <Type text="string" /> <MetaInfo text="optional" />
  - The name of the Preview to inspect. Defaults to the current git branch.

---

### `preview secret bulk`

Upload multiple secrets to a Preview. This patches the named Preview's latest deployment and creates a new deployment that goes live immediately. Setting a key to `null` in JSON format deletes that secret, matching the behavior of `wrangler secret bulk`.

```txt
wrangler preview secret bulk [FILE] [OPTIONS]
```

- `FILE` <Type text="string" /> <MetaInfo text="optional" />
  - The file of key-value pairs to upload, as JSON or `.env` format. Reads from stdin if omitted.
- `--name` <Type text="string" /> <MetaInfo text="optional" />
  - The name of the Preview to update. Defaults to the current git branch.
- `--message` <Type text="string" /> <MetaInfo text="optional" />
  - A descriptive message for this deployment.
...
Add or update a secret in the base configuration.

```txt
wrangler preview base-config secret put SECRET_NAME [OPTIONS]
```

- `SECRET_NAME` <Type text="string" />
  - The name of the secret to add or update.
- `--apply-to-existing` <Type text="boolean" /> <MetaInfo text="optional" />
  - Apply the base secret change to Previews that already exist, instead of new Previews only.

For example:

```sh
npx wrangler preview base-config secret put API_TOKEN
```
...
Delete a secret from the base configuration.

```txt
wrangler preview base-config secret delete SECRET_NAME [OPTIONS]
```
...
List secrets in the base configuration.

```txt
wrangler preview base-config secret list [OPTIONS]
```
...
Upload multiple secrets to the base configuration.

```txt
wrangler preview base-config secret bulk [OPTIONS]
```

- `--apply-to-existing` <Type text="boolean" /> <MetaInfo text="optional" />
  - Apply the base secret changes to Previews that already exist, instead of new Previews only.

Existing Previews keep their own settings unless you also apply the change to existing Previews.
...
This action creates a new [version](/workers/versions-and-deployments/#versions) of the Worker and [deploys](/workers/versions-and-deployments/#deployments) it immediately. To only create a new version of the Worker, use the [`wrangler versions secret`](#versions-secret-put) commands.
...
<WranglerCommand
	command="secret bulk"
	headingLevel={3}
	description="Create, update, or delete multiple secrets for a Worker in a single request, with up to 100 secrets per command."
/>
...
<WranglerCommand
	...
	description="Upload a new [version](/workers/versions-and-deployments/#versions) of your Worker that is not deployed immediately."
	headingLevel={3}
/>

<WranglerCommand
	command="versions deploy"
	description="Deploy a previously created [version](/workers/versions-and-deployments/#versions) of your Worker all at once or create a [gradual deployment](/workers/versions-and-deployments/gradual-deployments/) to incrementally shift traffic to a new version by following an interactive prompt."
	headingLevel={3}
/>
...
[Deployments](/workers/versions-and-deployments/#deployments) track the version(s) of your Worker that are actively serving traffic.
